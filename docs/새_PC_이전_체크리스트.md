...
### ⚠⚠ git 에 없는데 **계획이 거기 들어 있다** — `Claude outputs/`

총괄 브리프 32개(md)가 `Claude outputs/` 에 있고 **전부 미추적**이다.
`.gitignore` 로 막힌 것이 아니라 그냥 커밋한 적이 없다.
    ...
    제출폼_붙여넣기.md      (이건 .gitignore 에 걸려 있다)

**안 가져가면 무엇을 할지 모른다.** 리포 문서(`미완_목록.md`·`작업로그_*.md`)에
결론은 적혀 있지만 **지시 원문은 없다.** USB·클라우드로 폴더째 옮긴다.

⚠ 커밋해서 옮기고 싶어지는데, 그 폴더에는 `제출폼_붙여넣기.md` 처럼 이미
  `.gitignore` 에 넣어 둔 것이 섞여 있다. 폴더째 커밋하지 말 것.
