...
| `DOMEGGOOK_API_KEY` | 32자 | 표본 수집 스크립트가 안 돈다. **배포본은 안 쓴다** |

⚠ `DOMEGGOOK_API_KEY` 는 **호출 IP 가 등록된 키**다. 새 PC 로 옮기면 공인 IP
가 바뀌어 401/403 이 난다 - 도매꾹에 새 IP 등록을 신청해야 한다.
...
1. **배포** — 프로덕션이 9/2 빌드다. 오늘 코드 2건(`7a28637` 사유별 UNKNOWN
   헤드라인, `92b01ac` 부속서 단서 문구)이 안 올라가 있다.
