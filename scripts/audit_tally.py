...
# ── 기준선 표는 `sourcing_guard/baseline.py` 로 옮겼다 (2026-09-13) ──────
#
# ⚠ **값은 그대로다.** 랜딩이 70.4%·0건을 그 자리에서 그리려면 서버가 읽을 수
#   있어야 하는데, 앱이 `scripts/` 를 import 하는 것은 방향이 거꾸로다.
#   리터럴은 패키지에 두고 여기서 재수출한다 - `audit_tally.BASELINE` 이라는
#   이름은 그대로 닿는다(기존 import 스무 곳 · `[기준선 변경]` 커밋 규칙).
from sourcing_guard.baseline import (  # noqa: E402  (표 정의부라 위치를 지킨다)
    BASELINE,
    BASELINE_BATCH,
    BASELINE_EXTRACTOR,
    BASELINE_MATCH,
...
# ⚠ `batch_tally` 는 **도구**다. `tests/fixtures/` 를 읽으므로 앱 패키지에
#   두지 않는다 - 배포 이미지가 테스트 자료를 읽게 되면 그 자체가 결함이다.
#   상수만 패키지로 옮겼고 이 함수는 여기 남는다.
...
__all__ = [
    "BASELINE", "BASELINE_BATCH", "BASELINE_EXTRACTOR", "BASELINE_MATCH",
    "BASELINE_ON_VAGUE", "batch_tally", "compare_baseline", "load_audit", "load_correct",
    "load_reviewed_pairs", "load_scope", "tally", "verdict",
]
