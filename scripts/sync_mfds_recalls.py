...
#: 정상 응답 코드.
#:
#: ⚠ **아직 실제로 못 봤다 (R5).** 2026-09-14 시점에 받은 코드는 `ERROR-503`
#:   (시간 제한)뿐이다. `INFO-000` 은 식약처 OpenAPI 의 관례값이고 우리가
#:   확인한 값이 아니다 - 첫 성공 수집에서 실제 코드를 보고 이 줄을 고친다.
#:   **모르는 코드는 성공이 아니므로**(R3) 틀려도 조용히 저장되지 않고 멈춘다.
OK_CODES: tuple[str, ...] = ("INFO-000",)

#: 한 번에 받는 행 수. 식약처 상한은 확인 전이라 작게 시작한다 (R5).
PAGE = 100
