...
-----------------------------------------
랜딩이 "77.0% · 사람이 검수한 135건" 과 "0건" 을 **그 자리에서 그려야** 한다
...
#: ⚠⚠ **2026-09-14 [기준선 변경] — 미검수 18줄을 검수했다.** 다섯 숫자가
#:   움직였고 **정답이 줄었다.** 올린 것이 아니라 내린 것이다.
#:
#:       GPT(기준)   ① 95 → 104   ② 18 → 0   ③ 113 → 104   ④ 0   ⑤ 3
#:       Claude(대조) ① 99 → 105   ② 14 → 0   ③ 113 → 105   ④ 0   ⑤ 1
#:       배치         ① 96 → 103   ② 15 → 0   ③ 111 → 103   ④ 0   ⑤ 2
#:
#:   ③ 상한이 ① 과 같아졌다 - 미검수가 0 이므로 "미검수를 다 맞았다고 치면"
#:   이라는 가정이 없어졌다. **113(83.7%)은 더 이상 말할 수 있는 수가 아니다.**
#:
#:   무엇이 줄였나: 모기장·기저귀 가방·책가방 8줄에 「의류 이외의 섬유제품」
#:   (안전기준준수)이 붙어 있었는데, 고시는 **사용 연령으로** 가른다 -
#:   36개월 미만은 「유아용 섬유제품」(안전확인), 36개월~만 13세는 「아동용
#:   섬유제품」(공급자적합성확인)이다. 6줄 오답 · 2줄 애매.
#:   근거와 일련번호는 `tests/fixtures/새표본235_오답.tsv` 의 '연령 구간' 절.
#:   (CLAUDE.md R5-b - 갈리는 것은 고시가 정한다)
BASELINE: dict[str, dict[str, int]] = {
    # Claude · 단건 · 상품명만 · 분모 대상 135 · 커밋 e61ce1e 되돌린 뒤
    #
    # ⚠ **2026-09-11 부터 대조군이다.** 기준은 `BASELINE_EXTRACTOR` 쪽이고
    #   이쪽은 "추출기를 바꿔도 등급 결과가 크게 안 흔들린다"(검수 뒤 105 vs
    #   104 · 1건 차이)의 증거로 남긴다. 지우지 않는다.
    #   ⚠ 전에는 "상한 둘 다 113" 이라고 적었다. 그 113 은 미검수를 다 맞았다고
    #     친 수였고, 검수해 보니 둘 다 내려갔다.
    #
    #   ⚠⚠ **2026-09-20 부터 Claude 는 순서에서 빠졌다** (R7 개정 · Anthropic
    #     잔액 0). 이 값은 **과거 측정 기록**이고 라이브 대조군이 아니다.
    #     **지우지 않는다** - 추출기를 바꿔도 등급 결과가 크게 안 흔들린다는
    #     증거(105 vs 104 · 1건 차이)는 이것뿐이고, 본선에서 2순위를 되살릴 때
    #     다시 대조할 값이다.
    "claude": {
        "denominator": 135,
        "ok": 105,           # 검수된 쌍만 (77.8%)
        "ok_upper": 105,     # 미검수 0 이라 상한이 곧 정답이다
        "unreviewed": 0,
        "vague": 4,
        "wrong": 7,
        "missed": 19,
