...
def normalize_kc(raw: str) -> str:
    ...
    # ⚠⚠ **순서가 결과를 바꾼다.** 구분자(`:`·공백)를 **먼저** 지운다.
    #
    #   나중에 지우면 '인증번호:KC12345' 가 이 루프 뒤에 ':KC12345' 가 되고,
    #   KC 가 맨 앞이 아니게 되어 아래 접두 조건이 빗나간다 - 결과가
    #   'KC12345' 로 남는다. 2026-09-22 에 경계를 붙이면서 실제로 그렇게
    #   깨뜨렸고, 재서 잡았다.
    #
    #   ⚠ 전에는 "KC" 가 이 목록 안에 있어서 **순서가 무의미했다.** 지금은
    #     의존이 생겼으므로 정렬하거나 재배치하지 말 것. 회귀 가드는
    #     `test_kc_marker_is_stripped_only_when_it_is_a_marker` 다 (순서를
    #     되돌리면 2건이 실패한다).
...
# 근거로 쓸 수 없는 리콜 원문 주소.
#
# ── 실측 (로컬 사본 37,313건, 2026-09-01) ──────────────────────────────
#
#   국내 4,243건    detail_url 있음      0 (  0.0%)   응답에 필드 자체가 없다
#   국외 33,070건   detail_url 있음 18,313 ( 55.4%)
#                   그 중 근거로 사용 가능 15,519      나머지 2,794 는 URL 이
#                   아니었다 - '-' 이거나 제품 설명문이 그 칸에 들어와 있다
#
#   표본 900건을 실제로 열어봤다. 봇 차단(403)·TLS·DNS 로 못 연 68건을 빼고
#   측정된 832건 기준:
#
#       live 99.0%   dead 1.0%   메인페이지로 리다이렉트 0.0%
#
#   즉 국외 원문 링크는 만료가 문제가 아니다(2019~2026년분 전부 그렇다).
#   문제는 부재다 - 전체 37,313건 중 21,794건(58.4%)이 쓸 주소가 없어서
#   지금까지 메인페이지로 떨어지고 있었다.
#
#   ⚠ 첫 측정은 '오류 52.1% · 죽음 13.2%' 였는데 전부 계측 쪽 문제였다.
#     cpsc.gov 가 우리 UA 를 403 으로 막았고(브라우저 UA 로는 8/8 이 200),
#     ec.europa.eu·rappel.conso.gouv.fr 은 이 PC 의 신뢰 저장소에 CA 가 없어
#     SSLCertVerificationError 였다. "우리가 못 열었다" 를 "없어졌다" 로
#     읽으면 멀쩡한 근거 링크를 걷어내게 된다.
#
# 그래서 걸러내는 것은 네트워크 없이 판별되는 구조적 결함뿐이다:
#
#   · 빈 값 / 공백                     국내 리콜은 이 필드 자체가 없다
#   · http(s) 가 아닌 스킴             javascript:, mailto:, 상대경로,
#                                      그리고 URL 이 아닌 설명문 2,794건
#   · 경로 없는 호스트 루트            "https://www.cpsc.gov/" 는 그 리콜을 가리키지
#                                      않는다. 메인페이지는 근거가 아니다.
#   · 알려진 메인페이지 경로           /index.html, /main, /home 등
#
# 살아 있는지는 런타임에 재지 않는다. 스캔마다 외부로 요청을 날리면 응답이
# 그 사이트 지연에 묶이고, 외국 기관 서버에 우리 트래픽이 그대로 간다.
# 다시 잴 때는 scripts/probe_recall_urls.py 를 쓴다.
_DEAD_PATHS = {
