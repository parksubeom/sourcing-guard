...
#:
#:   그래서 (1) 시도는 **닫힌 목록**으로, (2) 토막 사이에 공백을 요구하고,
#:   (3) 마지막에 로/길/읍/면/동/리를 요구한다. `전남 구례군 산수유` 같은
#:   원산지 표기는 도로명이 없어 안 걸린다 - 그것은 개인정보가 아니다.
#:
...
#:   텍스트에 섞여 든 주소**만 잡는다.
#: ⚠⚠ **두 갈래다** (2026-09-14 2차 · 총괄 실측). 시도를 **요구**하면
#:   `성남시 분당구 판교로` 처럼 시도를 생략한 주소가 새고, 시도를 그냥
#:   **선택**으로 두면 `제조일자 **표시** 식품**으로** 해당사항 없음` 이
#:   '시군구 + 도로명' 으로 읽힌다(실측 5건 · 황색포도상구균과 같은 모양).
#:
#:   가르는 것은 **접미사 앞 글자 수**다. 행정구역은 최소 두 글자이고
#:   (강남구·분당구·성남시·구례군) `표시` 는 한 글자다. 다만 `부산 중구` ·
#:   `서울특별시 중구` 처럼 **한 글자 구가 실재**하므로 시도가 있을 때만 연다.
#:
#:       시도가 있으면   시군구 1자 이상 허용   (부산 중구 중앙대로)
#:       시도가 없으면   시군구 **2자 이상** 요구 ('표시' 를 배제)
_SIDO = (
    r"(?:서울|부산|대구|인천|광주|대전|울산|세종|경기|강원|충북|충남|전북|전남|경북|경남|제주)"
    r"(?:특별시|광역시|특별자치시|특별자치도|특별자치|북도|남도|도)?"
)
_ROAD = r"[가-힣0-9]{1,10}(?:로|길|읍|면|동|리)\b"
_ADDR = re.compile(
    rf"(?:{_SIDO}\s*(?:[가-힣]{{1,6}}[시군구]\s+){{1,2}}{_ROAD})"
    rf"|(?:[가-힣]{{2,6}}[시군구]\s+){{1,2}}{_ROAD}"
)
