...
class ResultAxis(BaseModel):
    ...
    note: str = ""      # 기준일 같은 한정
    #: 「지금까지 확인한 것」 상자에 쓸 메모. **행위만** 적는다.
    #:
    #: 주의(가장 중요): 그 축에 주의·위험 finding 이 붙어 있으면 여기는
    #:   "결과는 아래" 다 - 초록 바탕에 "인증상태: 기간만료" 가 있으면 읽는
    #:   사람이 "만료인데 왜 초록?" 하고, 바로 아래 상자에 같은 말이 또 있다
    #:   (사양 §2-④ · 2026-09-21). `note` 는 그대로 둔다 - 축 카드에서는
    #:   결과가 필요하다.
    did_note: str = ""
...
NON_SPECIFIC_FINDING_KINDS: frozenset["FindingKind"] = frozenset({
    ...
    #
    # ⚠⚠ **2026-09-11 정정 — "늘 붙는다" 는 더 이상 사실이 아니다 (4-r).**
    #
    #   왜 늘 붙었는지를 그때 묻지 않았다. 답은 `verifier` 가 대조 가능 여부를
    #   `RecallIndex` 와 **다른 조건으로 판단**해서, `product_name` 만 있어도
    #   "대조했고 없었다" 를 말했기 때문이다. **대조를 안 하고 붙은 것**이다.
    #
    #   그 결함을 고치니 이제 URL 만 붙여넣은 검사에는 안 붙는다:
    #
    #       "https://example.com/goods/1"  →  coverage_gap · info_request
    #       "유아용 블록 완구 …"            →  (모델명·제조사 없으면) 안 붙음
    #
    # ⚠ **그래도 여기 둔다. 근거가 둘에서 하나로 줄었지만 그 하나가 R3-b 다.**
    #
    #   "리콜 공표 목록에서 못 찾았다" 는 **부재의 증명**이고, 부재를 근거로
    #   안심시키지 않는다는 것이 불변 규칙이다. 공표되지 않은 결함은 대조할 수
    #   없고(기획서 §3.2), 로컬 사본이라 오늘 공표된 것은 다음 동기화 전까지
    #   안 잡힌다.
    #
    #   **근거가 하나여도 그것이 R 규칙이면 충분하다.** SPECIFIC 으로 옮기는
    #   것은 "대조했는데 없었다" 를 셀러에게 준 값으로 세는 것인데, 그것이
    #   정확히 R3-b 가 금지하는 반올림이다. 2026-09-11 판정으로 닫혔다.
...
class ScanResult(BaseModel):
    ...
    recall_data_as_of: str | None = None
    # ⚠⚠ **공표일과 갱신 시각은 다른 것이다.** 공표일만 적으면 셀러가
    #   "3일 전 데이터" 로 읽는데, 주말·공휴일에는 정부 공표가 없어서 공표일이
    #   며칠 전인 것이 정상이다. "우리가 언제 받아 왔나" 를 따로 적어야 그
    #   오해가 안 생긴다.
    #
    # ⚠ 2026-09-20 에 **축 메모에서 여기로 옮겼다.** 축은 "일치 항목 없음 ·
    #   2026-09-17 공표분까지" 두 절이면 충분하고(찾은 것이 앞, 출처가 뒤),
