...
#   실패하는 최악의 편향이 된다.
#
# ⚠ 15초도 부족했다. 직접 재면 11.7~11.9초로 일정한데 앱에서 돌리면 15초를
#   넘겨 ReadTimeout 이 났다(실측). category=R 은 22.5초까지 갔다. 실패하면
#   캐시에 아무것도 안 남아 다음 클릭도 같은 시간을 쓰므로, 느리게라도 답이
#   나오는 쪽이 낫다. 화면 버튼이 소요 시간을 미리 알린다.
