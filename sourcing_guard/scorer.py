...
_PENALTY: dict[FindingKind, int] = {
    ...
    # ── 안내 축 셋. 전부 0 이다 (2026-09-14) ───────────────────────────
    #
    # ⚠⚠ **이 셋이 신호를 바꾸면 안내가 판정이 된다.** "호환" 이라는 말이
    #   적혔다고 노란불을 주면, 정품 액세서리를 파는 셀러가 매번 노란불을 받고
    #   노란불 자체를 무시하게 된다 - SUBSTANCE_MENTIONED 를 25 로 둔 것과
    #   반대쪽 판단이고, 그 차이는 **우리가 조회했는가**에서 온다. 규제 물질
    #   표기는 우리 규칙 DB 와 대조한 결과이고, 표기어는 대조한 것이 없다.
    #
    # ⚠ `origin_mismatch` 도 0 이다. 정부 DB 와 상세페이지가 다르다는 사실은
    #   확인할 거리이지 **어느 쪽이 틀렸는지 우리가 정한 것이 아니다.** 수입
    #   제조국 표기가 여러 개이거나 OEM 생산지가 바뀐 경우가 실제로 있다.
    #   깎으려면 "정부 DB 가 문제를 적어뒀는가" 를 먼저 물어야 하고(R3-b),
    #   불일치는 그 조건을 만족하지 않는다.
    FindingKind.IP_MARKER_NOTICE: 0,
    FindingKind.JURISDICTION_NOTICE: 0,
...
def _axes(
    ...
    if cert[1]:
        ...
        # 주의(가장 중요): **여기서도 원인을 말하지 않는다** (P2 · 2026-09-20).
        #   전에 "잠시 후 다시 시도해 주세요" 였다. 문장 쪽은 P2 에서 고쳤는데
        #   **축 카드 쪽이 남아 있었다** - 같은 화면에서 문장은 원인을 안 말하고
        #   바로 옆 카드는 "잠시" 라고 단정하고 있었다. 우리는 그것이 잠시인지
        #   모른다 (P6 실측: 도쿄에서 계속 막혀 있다).
        #   ⚠ 검사가 이 줄을 보게 했다 - `test_lookup_honesty` 가 문장만 보고
        #     있어서 못 잡았다. 틀이 아니라 틀에 붓는 값을 본다 (§6).
        cert_note = "근거 링크에서 직접 조회하실 수 있습니다"
    elif cert[0] == "번호 없음":
