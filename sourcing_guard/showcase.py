...
⚠ `certs[].detail_url` 2건은 `_pick_exact`(2026-09-22) **이전 코드의 산물**이다
  (`CB063R4501-0001 → …-0001A` · `cb064r2424-9001 → …-9001R`). collect(9/20)
  단계가 만든 수집 메타이고 `scan` 이 안 건드린다. **화면에 나가지 않는다** -
  목록 카드도 단건 응답도 `certs` 를 안 낸다(라이브 확인). 재수집 때 사라진다.
  세는 도구는 `scripts/audit_recorded_certs.py` 이고 이 둘을 알려진 예외로 둔다.
