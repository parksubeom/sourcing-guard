...
⚠⚠ **상품 제목은 손대지 않는다 — 체험 표본과 일부러 다르다** (2026-09-20 총괄 판정).

  `scripts/record_samples.py` 는 제목 앞머리의 `[상호]` 괄호 묶음을 뗀다.
  여기서는 **떼지 않는다.** 조건이 다르기 때문이다:

      체험 표본   우리가 고른 상품을 **우리 화면 안에서만** 보여 준다.
                  상호가 남을 이유가 없다
      showcase    카드가 **도매꾹 원문으로 링크한다.** 누르면 같은 제목이
                  같은 상호와 함께 그 페이지에 있다

  링크를 걸어 두고 제목만 가리는 것은 **가리는 시늉**이고, 고치면 "실제
  도매꾹 판매중 상품" 이라는 화면의 주장이 거짓이 된다.

  ⚠ 우리가 저장하지 않기로 한 것은 `id`·`nick`·`seller` **필드**이지
    제목이 아니다. 그 구분이 이 모듈의 남길 목록 그대로다.

  ⚠ 제목에 `(주)`·`주식회사`·`㈜` 가 든 상품이 다음 수집에 들어올 수 있다
    (오늘 29장에는 없다). 그때도 **고치지 않는다** - 위 이유가 그대로 적용된다.
    바꾸려면 링크를 먼저 떼야 하고, 링크를 떼면 이 화면의 값이 사라진다.
...
우리 값도 목록에 없으면 버린다 - 그래야 "남길 목록" 이 한 겹이 아니다.
    ...
    certs        국표원 조회 결과 (**번호마다 한 줄**. 번호 · 상태 · 원문 링크)
