...
async def sync_loop(
    ...
    *,
    ...
    ⚠ 리콜 다음에 돌린다. 리콜 대조가 그동안 막히면 안 된다.
      sync_noncompliant 는 예외를 밖으로 던지지 않으므로 실패해도 루프가 죽지 않는다.

    ⚠⚠ **여기 걸리는 시간이 곧 다음 회차를 미룬다.** `sleep(interval)` 이 이
      **뒤에** 오므로 실제 주기는 `run_sync + RRA + interval` 이다. 시각으로
      다음 회차를 계산하면 틀린다 - 2026-09-23 에 총괄이 그렇게 셌다가
      「09:24:36Z 배포」 시한을 6분 일찍 잡았고, 문자 그대로 따랐으면 자고 있던
      루프를 죽여 하루 기다린 관측을 또 못 볼 뻔했다 (§1-ai).

    ⚠ 걸리는 시간이 **회차마다 다르다.** 실측:

          2026-09-22  약 6분   (전에 여기 "275페이지 순차 수집 · 약 5분" 이라
                                적혀 있던 그 모양)
          2026-09-23  **7초**  (rf synced_at 09:48:52 = last_sync_at + 7초 ·
                                count 2753 → 2759)

      원인은 안 팠다. 아마 변경분만 받는 경로가 있거나 서버 응답이 달랐을 것이다 -
      **모르는 채로 적는다** (R5). 여기 상수를 적어 두면 그 수가 다음 사람에게
      거짓말을 한다 (§6 "기록에 움직이는 값을 박아 넣지 않는다").
    """
