...
#: 연속 실패 임계. **이 수를 넘겨야 "계속 실패" 라고 말한다.**
#:
#: ⚠⚠ **이 3 은 우리가 정한 값이지 측정값이 아니다** (R5-b ③ - 정했다는
#:   사실과 근거를 같이 적는다). 근거는 구조뿐이다 - 한 번의 스캔이 인증·리콜
#:   두 축을 치므로 2 로 두면 한 번 끊긴 것도 "계속" 이 된다. 3 이면 최소
#:   두 번의 시도가 필요하다.
#:
#: ⚠ **재는 자리는 투표 기간이다.** 매일 `/healthz` 의 `kats
#:   .consecutive_failures` 가 어떻게 오르내리는지 보면 3 이 이른지 늦은지가
#:   보인다. 그때까지 이 수를 근거로 다른 판단을 세우지 말 것.
#: ⚠ 재시도가 아니다. 세기만 한다 (`kats_client` 133행 주석).
_PERSISTING_AFTER = 3


def _lookup_is_persisting() -> bool:
    """정부 조회가 **연이어** 실패하고 있는가.

    ⚠ `kats_client.health` 는 프로세스 메모리다. 재배포하면 0 이고, 그건
      "지금은 모른다" 라서 한 줄을 안 붙이는 쪽으로 떨어진다 - 맞는 방향이다.
    """
    from .kats_client import health

    return health.consecutive_failures >= _PERSISTING_AFTER
...
def _lookup_failed(
    ...
    ⚠⚠ **원인을 말하지 않는다** (2026-09-20 P2).

      전에는 "**일시적으로** 연결하지 못했습니다 … **잠시 후** 다시 시도해
      주세요" 였다. 한 문장에 원인 단정이 둘이다.

      우리는 원인을 모른다. P1 실측이 `network.connect` 까지는 좁혔지만
      (도쿄에서 TCP 핸드셰이크가 30초에도 안 붙는다), **도쿄만인지 국외
      전반인지는 아직 안 쟀다.** 그리고 2026-09-18 에 같은 도쿄에서 스스로
      풀린 기록이 있어 "계속 실패한다" 도 단정할 수 없다.

      R3 이 답을 갖고 있다 - *"데이터가 없으면 UNKNOWN 이다. 「아마 괜찮을
      것」이라는 추정은 코드에도 프롬프트에도 넣지 않는다."* **원인 추정도
      같다.** 사실 + 우리가 실제로 센 것 + 항상 참인 다음 행동만 적는다.

    키 무효(4000)·IP 미등록(4001) 은 다르다 - 그건 **우리 설정 문제이고 우리가
    안다.** 아는 것은 말한다.

    ⚠⚠ **이 규칙을 저장소 전체로 넓히지 마라** (총괄 판정 2026-09-20).

      P2 가 고친 것은 *"우리가 원인을 모르는데 원인을 말한 자리"* 하나다.
      「잠시 후」가 남아 있는 세 자리는 **우리가 원인을 아는 자리**라 그대로
      둔다:

          main.py 429 신고 과다     1분 뒤면 진짜 풀린다 - 우리 리미터다
          main.py 등급표 읽기 실패   우리 쪽 문제이고 재시도로 낫는다
          index·watch fetch 접미사  순간 네트워크 오류다

      다음 사람이 **"일관성"** 을 이유로 셋 다 바꾸려 한다. 그러면 429 에서
      "다시 시도" 를 못 말하게 되고, 그건 셀러에게 손해다. 갈림선은 문구가
      아니라 **우리가 그 원인을 아는가** 다.
    """
    # ⚠⚠ **이번 코드와 전역 상태를 둘 다 본다** (2026-09-20 P2).
    #
    #   전에는 "전역 상태가 아니라 이번 오류의 코드로 판단한다. 전역을 보면
    #   직전에 다른 종류의 실패가 있었을 때 엉뚱한 문구가 나간다" 였다.
    #   그 걱정은 맞다 - 그래서 **버리지 않고 조건에 넣었다.** 이번 코드가
    #   `network*` 이고 **동시에** 연속 실패가 임계 이상일 때만 한 줄을 더한다.
    #   둘 중 하나만 보면 그 걱정이 그대로 돌아온다.
    #
    # ⚠ 더하는 한 줄은 **원인이 아니라 우리가 센 수**다. "계속 실패하고 있다"
    #   는 관측이고, "지역 때문이다" 는 추정이다.
    if code in OPERATOR_FAULT_CODES:
        tail = " 조회 서비스 설정을 점검하고 있습니다."
    elif (code or "").startswith("network") and _lookup_is_persisting():
        tail = " 최근 조회 시도가 계속 실패하고 있습니다."
    else:
        tail = ""
    return Finding(
        ...
        statement_ko=(
            f"{agency} {what} 조회 서비스에 연결하지 못했습니다. "
            f"{what} 확인이 완료되지 않았습니다.{tail} "
            "아래 근거 링크에서 번호를 직접 조회하실 수 있습니다."
        ),
...
def verify(
    ...
    if facts.category is ItemCategory.OUT_OF_SCOPE:
        ...
        #
        #   처음 문구는 "다른 법의 소관일 가능성이 있어 … 판단하지 않았습니다"
        #   였다. 그런데 이 finding 이 붙는 12건 중 **2건이 대상으로 검수된
        #   줄**이었다 - [CU] 핏미업 러닝벨트(부속서 1 기타 제품류) ·
        #   [202] 지압슬리퍼. 그 줄에서 "다른 법 소관" 은 **틀린 정보**이고,
        #   셀러를 전안법에서 멀어지게 한다. 없는 의무를 만드는 것의 거울이며
        #   우리 오류 비대칭에서 **더 비싼 쪽**이다.
        #
        #   그렇다고 "판단하지 못했습니다" 로 일반화하면 `coverage_gap` 과
        #   중복이다 - 침묵 줄에는 이미 그것이 붙어 있다. 이 finding 의 몫은
        #   **두 갈래를 명시하는 것**이고, 그것이 실제 우리 앎의 상태다 (R3).
        #
        # ⚠ "이 표가 놓친 것일 수도 있습니다" 는 **자기 한계 표기**이지 내부
        #   사정 노출이 아니다. 금지되는 것은 "추출기가 헷갈렸다" 처럼 우리
        #   구현을 셀러에게 설명하는 것이고, "우리 표에 없다" 는 셀러가 다음
        #   행동을 정하는 데 쓰는 사실이다.
        if not scope_reason:
            findings.append(
                Finding(
                    ...
                    signal=Signal.UNKNOWN,
                    statement_ko=(
                        "이 상품은 이 도구의 등급표에서 찾지 못했습니다. "
                        "다른 법의 소관일 수도 있고, 이 표가 놓친 것일 수도 "
                        "있습니다. 어느 쪽인지는 특정하지 못했습니다. "
    ...
    #
    #   OUT_OF_SCOPE  "우리 소관 아님" 이다. **계속 막는다.**
    #
    #   ⚠⚠ **2026-09-08 에 열었다가 2026-09-09 에 되돌렸다 (4-e → 4-e′).**
    #     다음 사람이 같은 이유로 다시 열려 할 것이므로 무엇이 막았는지 적는다.
    #     `tests/test_out_of_scope_grade_gate.py` 가 닫힌 상태를 잠근다.
    #
    #     **연 이유** — 도매꾹 고시 `type`(주방용품 등)이 추출기의 category 를
    #     여기로 밀어내고, **고시 품목분류와 전안법 품목군은 다른 분류다.**
    #     실측으로 상세 109 에서 [146] 방수매트 → 매트류, [165] 앞치마 → 의류가
    #     회복되고 정답이 89 → 91 이 됐다. `legal_item_name` 확장 폴백을 함께
    #     끄니 비대상 부착도 0 을 지켰다(끄지 않으면 `커피머신클리너 세정제`
    #     → `커피메이커` 로 0 → 1).
    #
    #     **되돌린 이유** — 통과 기준에 없던 숫자가 움직였다.
    #     **애매 부착 1 → 2** (Claude 재생 · GPT 는 3 → 4). 늘어난 줄은
    #
    #         [26] 아임명작 청정 북유럽 에어프라이어 종이호일 → 전기오븐기기
    #
    #     26번은 2026-09-07 에 "종이호일이면 식약처 소관이고 '에어프라이어' 는
    #     용도어라 전기오븐기기는 오답" 으로 판정해 대상 → 애매로 옮긴 줄이다
    #     (새표본235_대상분류.tsv 머리 주석). 즉 **LLM 의 out_of_scope 가
    #     맞았던 경우**를 게이트가 뚫었다. 분모 밖이라 정답률에는 안 보이지만
    #     화면에서 종이호일이 '전기오븐기기 · 안전인증' 을 받는다 - 비대상 0 과
    #     같은 종류의 오부착이다.
    #
    #     **가를 방법을 찾지 못했다.** 26 과 146·165 를 나눌 신호를 넷 쟀다:
    #
    #         매칭 출처            셋 다 LLM 의 product_name · matched_by=alias
    #                              (`raw_text` 는 관여하지 않는다 - "원본에만
    #                               있는 키는 버린다" 규칙이 아무것도 바꾸지
    #                               않는다. 실측으로 확인했다)
    #         걸린 키              26 '에어프라이어' · 146 '방수매트' · 165 '토시'
    #         out_of_scope_reason  셋 다 None
    #         has_consumable_hint  셋 다 False
    #
    #     상한 +1 · 상세 정답 +2 를 얻으려고 애매 오부착을 사는 거래는 하지
    #     않는다. 26번의 '에어프라이어' 가 **용도어**라는 것을 가리는 규칙
    #     (소모품 ↔ 본체)이 생기면 그때 다시 연다.
    #
    #     ⚠ 옛 근거는 재현되지 않는다 - "정수기컵(식약처)·목발형 보행기
    #       (의료기기)에 전안법 등급을 붙이면 오답" 이라 적혀 있었는데 실측하니
    #       둘 다 후보 0건이다. 게이트를 닫아 두는 근거는 그것이 아니라 위
    #       26번이다.
    #
    ...
    #
    # ⚠⚠ **별칭 게이트(`item_grades` gate_text)는 원본까지 본다. 일부러 다르다.**
    #
    #   같은 신호를 두 게이트가 다른 입력에서 읽는 것이 §6("같은 판단을 두 곳에
    #   적지 마라")에 걸리는 것처럼 보이지만, **같은 판단이 아니다**:
    #
    #     별칭      등급표 **조회**를 넓힌다 → 틀려도 확인 화면이 뜬다 → 넓게
    #     포괄 규정  **의무를 새로 만든다**  → 틀리면 없는 의무를 만든다 → 좁게
    #
    #   통일하려면 재고 나서 한다. 2026-09-12 측정 - 별칭 쪽을 상품명으로 좁히면
    #   **진짜 상품 3건을 잃는다**(다섯 기준 ① 95→93 · 도매꾹 상세 정답 89→86):
    #
    #     [179] 산리오 포토카드보관겸용 필통 / **학용품보관**   잃음 '학용품'
    #     [181] 메쉬필통 … **초등**필통 **초등학생**필통        잃음 '학용품'
    #     [200] 왕골슬리퍼 … **EVA** … 실내화                  잃음 '신발류'
    #
    #   셋 다 표지어가 **원제목에 있고** LLM 이 정리하면서 떨어뜨렸다.
    #   **이 비대칭은 설계다.** 자세한 것은 `item_grades.py` gate_text 주석.
    #
            ...
            if rec is None:
                findings.append(
                    Finding(
                        ...
                        signal=Signal.AMBER,
                        # ⚠⚠ 「조회 DB에 **없는 것**이 정상」이라고 쓰면 안 된다.
                        #
                        #   2026-09-22 에 조회를 접두 부분일치에서 **정확 일치**로
                        #   바꾸면서 이 경로의 의미가 달라졌다. 전에는 응답이 통째로
                        #   비었을 때만 왔으므로 "DB에 없다" 가 참이었다. 지금은
                        #   **근처 값이 있어도** 여기로 온다 - 실측으로 셋 다 그랬다
                        #   (CB064R2424-9001 → …-9001r · …-9001 · …-9001A).
                        #   그 경우 "없는 것" 은 거짓이고, 거짓 설명은 덜 맞는 것이
                        #   아니라 틀린 것이다 (총괄 2026-09-22).
                        #
                        #   ⚠ 그렇다고 이 안내를 통째로 떼지 않았다. 이 문장이
                        #     R3-b 가 존재하는 이유다 - 미조회를 RED 로 읽지 않게
                        #     하는 것. 떼면 정상 SCoC 상품의 셀러가 놀란다.
                        #     대신 **DB 상태에 대한 주장**을 **조회 결과에 대한
                        #     서술**로 바꿨다. 근처 값이 있든 없든 참이다.
                        #
                        #   ⚠ "근처 값이 있었나" 를 갈라 말하려면 그 사실이 캐시를
                        #     지나야 한다 - 캐시가 재현 못 하는 값을 화면에 내면
                        #     캐시 적중과 신선 조회가 다른 문장을 말한다 (§6).
                        #     그건 별건이다.
                        statement_ko=(
                            f"상세페이지에 표기된 인증번호 '{num}'"
                            f"{subject_particle(num)} 조회되지 않습니다. "
                            "공급자적합성확인 대상 품목은 이 조회에서 확인되지 않을 "
                            "수 있으므로, 이 품목의 인증 구분을 먼저 확인해 주세요."
                        ),
...
def _kc_missing_finding(
    ...
    if grade in _ABSENCE_IS_NORMAL:
        ...
        return Finding(
            ...
            statement_ko=(
                ...
                f"({grade})에서는 부재가 정상이므로 문제로 보지 않았습니다. "
    ...
    if grade:
        # 등급을 특정했으니 일반론보다 세게 말할 수 있다 - 품목군이 아니라
        # 등급을 지목한다.
        #
        # 다만 "{grade} 대상이면 인증번호가 반드시 있어야 합니다" 는 여기서
        # 말하지 않는다. 바로 아래 ITEM_GRADE_MATCHED 가 품목명·범위 한정과
        # 함께 말하므로 두 번 읽게 된다.
        head = (
...
def _item_grade_findings(
    ...
    *,
    ...
    #
    # 손으로 만든 별칭이 한계에 왔다. 튜닝 표본(도매꾹239) 매칭률 71% 인데
    # 새 표본(235) 매칭률은 24% 였다 - 별칭을 만들 때 쓴 표본에서만 잘 듣는다.
    # ⚠ 둘 다 **검수 전 매칭률**이다. 발표에 쓰는 숫자는 새 표본 전수 검수
    #   **정답률 20%** 다 (tests/fixtures/새표본235_오답.tsv).
