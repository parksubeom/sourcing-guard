...
def test_the_proposal_numbers_match_the_code():
    ...
    #
    #   게다가 열던 파일이 `단건경로_136건.json`(68.4% 시절, e0b4b68)이라
    #   지금 원자료도 아니었다.
    #
    ...
    #   기준을 걸기로 했으므로(커밋 `d6e57a6`) 기획서 형식도 그래야 한다.
    ...
    # ── 미검수 분리를 **발표 숫자에도** 건다 ──────────────────────────
    #
    # ⚠ GPT 측정이 발표 숫자 쪽 구멍을 드러냈다. 83.7% 의 113건 중 일부는
    #   **검수 파일에 없는 쌍**이다 - 부속서 1 별칭(모기장·가방·장갑·토시·
    #   타월)이 원자료 측정(9/7 20:10) 뒤에 들어왔고, 그때 눈으로 검수해
    #   커밋 메시지에 적었지만 그것은 기록이 아니다.
    #
    ...
    #   두면 다른 회귀를 가리므로 사실을 잠그는 쪽을 골랐다 - 값이 바뀌면
    #   반드시 손대게 되므로 잊히지 않는다.
    ...
    #   하고, 문서가 코드보다 앞서 나가는 그 결함을 한 번 더 만든다.
    #   할 일은 docs/미완_목록.md 에 적어 뒀다.
    #   여기서 `"검수" in doc` 같은 낱말 검사를 걸면 통과만 하고 아무것도
    #   보장하지 않으므로 두지 않는다.
