...
def test_the_sweep_that_found_these_tables_is_written_down():
    ...
    # ⚠ 11 이다. 이력을 남긴다 - 처음 6 으로 적었다가 실측 7 이었고, [4-p] 의
    #   `gov_lookup_state` 가 인증(6개)·리콜(4개) 집합을 더해 9 가 됐다.
    #   2026-09-21 에 `_verified_counts` 의 RF 집합(10)과 `_ATTENTION_AXIS`(11)이
    #   더해졌다. **이 가드가 네 번 다 잡았다** - 표를 만들고 목록에 안 적는
    #   것을 막는 것이 목적이므로 정상 동작이다.
    #
    #   models 2    SPECIFIC · NON_SPECIFIC
    #   scorer 8    _PENALTY · _HARD_RED · _signal_for AMBER · _axes 인증
    #               · gov_lookup_state 인증 · gov_lookup_state 리콜
    #               · _verified_counts RF (전파 축이 돌았는가)
    #               · _ATTENTION_AXIS (주의가 어느 축에 걸리는가)
