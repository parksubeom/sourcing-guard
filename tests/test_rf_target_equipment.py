...
#: ⚠⚠ **넓히는 근거는 "찾은 것" 이 아니라 "품목명의 성질" 이다 (⑦-a-4).**
#:
#:   처음 정규식은 `^o\\s|(?:한다|된다)[\\s.]|제외한다` 였고 `LIT22` 를 놓쳤다:
#:
#:       "조광기(디밍) o 직류전원만을 사용하여 동작하는 기자재는
#:        적합성평가 대상에서 **제외**"
#:
#:   셋 다 안 걸린다 - `o` 가 문자열 **중간**이라 `^o` 가 아니고, 끝이 `제외`
#:   라서 `제외한다` 도 아니고, `한다/된다` 뒤에 공백·마침표가 없다.
#:
#:   ⚠ **정규식이 찾은 것에 맞춰 짜여 있었다.** 이번 주에만 네 번째 모양이다
#:     (`'포함'` 별칭 · `…come_from_the_notice…` 검사 · 내 손실 13 오독 · 이것).
#:     넓히는 쪽이 옳고 안전하다 - **품목명에는 `제외` 도 ` o ` 도 들어갈 수
#:     없다.** 지금 출력에 맞춘 것이 아니라 이름의 성질에서 나온 규칙이다.
#:     실측: 넓혀도 새로 걸린 것은 `LIT22` 하나뿐이다(오탐 0).
#:
#:   ⚠ `LIT22` 는 이 결함의 **가장 깨끗한 사례**다 - 진짜 이름 `조광기(디밍)`
#:     이 괄호까지 온전히 보이고 그 뒤에 제외 조항이 통째로 붙어 있다.
#:     디머 스위치는 셀러가 실제로 파는 물건이다.
_SENTENCE_NOT_A_NAME = re.compile(r"제외|(^|\s)o\s|(?:한다|된다)[\s.]")

#: 2026-09-14 실측. **줄이면 이 수도 줄여 잠근다.**
_SENTENCE_NAMES_TODAY = 23
#: 그중 examples 가 있어 **매칭에 닿는** 행. 여기가 진짜 위험한 자리다.
#: ⚠ `LIT22` 는 examples 가 없어 닿지 않는다 - 그래서 이 수는 2 그대로다.
